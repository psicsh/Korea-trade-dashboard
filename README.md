...
- 전체 무역: 최신 월 지표, 최근 12개월·5년 월별 그래프, 가용 전체 기간의 연도별 그래프, CSV 다운로드
- 20대 품목: 2026년 개편 MTI 기준 최신 월 비교, 품목별 12개월·5년 그래프, CSV 다운로드
- 9대 지역: 미국·중국·아세안·EU(27)·일본·중남미·인도·중동·CIS 비교 및 시계열
- HS 상세조회: 숫자 2·4·6자리만 허용, 조회 버튼을 누를 때만 호출, 12개월·5년 선택, 공식 명칭·관련 MTI·CSV 제공
...
5. GitHub의 `Actions → 관세청 무역자료 갱신 → Run workflow`에서 `mode=bootstrap`을 한 번 실행합니다.
...
관세청 공식 명세에서 `hsSgn`과 `cntyCd`는 선택 항목입니다. 최초 구축은 다음 방식으로 호출량을 줄입니다.

- 전체 무역: 10년 단위로 나누어 장기 시계열 수집
- 9대 지역: 국가별 전체 자료를 최대 12개월 단위로 한 번씩 받고 지역 정의에 따라 합산
- 20대 품목: 한 달의 전체 HSK를 한 번만 받은 뒤 공식 2026 HSK–MTI 연계표로 집계
- 월별 갱신: 새 달과 직전 2개월만 다시 검증
- HS 상세조회: 사용자가 입력한 코드만 실시간 조회

최초 품목 구축은 약 55회의 대용량 호출이 필요해 시간이 걸릴 수 있지만, 이후 월별 갱신은 최근 3개월만 처리합니다.
